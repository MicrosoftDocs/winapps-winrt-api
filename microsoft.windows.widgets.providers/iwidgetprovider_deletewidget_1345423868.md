...
The unique identifier of the widget that is being unpinned. The widget ID value is dynamically generated by the [WidgetManager](/windows/windows-app-sdk/api/winrt/microsoft.windows.widgets.providers/widgetmanager). The widget ID remains the same for a widget from the moment of its creation until the moment the Widget is deleted. The widget ID is a unique value across all widgets and all widget providers. 
...
As soon as this method has been called for a specific widget ID, your provider will not receive any other requests for that widget. At the time this method is called, using the deleted widget's Id to make any further calls on the methods of the [WidgetManager](/windows/windows-app-sdk/api/winrt/microsoft.windows.widgets.providers/widgetmanager) will have no effect.
