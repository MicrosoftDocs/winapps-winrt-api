---
-api-id: P:Microsoft.Windows.Widgets.Providers.WidgetAnalyticsInfoReportedArgs.WidgetContext
-api-type: winrt property
---

# Microsoft.Windows.Widgets.Providers.WidgetAnalyticsInfoReportedArgs.WidgetContext

<!--
public Microsoft.Windows.Widgets.Providers.WidgetContext WidgetContext { get; }
-->


## -description

Gets the [WidgetContext](/windows/windows-app-sdk/api/winrt/microsoft.windows.widgets.providers.widgetcontext) object associated with the analytics info. This object provides information about the configuration of a widget within the widget host.

## -property-value

The **WidgetContext** object.

## -remarks

## -see-also

## -examples


