...
UserControl does not manifest the value of its `Background` property in its control template. Instead, set the `Background` of the root element inside the UserControl. For more info, see Remarks in [Control.Background](control_background.md).
