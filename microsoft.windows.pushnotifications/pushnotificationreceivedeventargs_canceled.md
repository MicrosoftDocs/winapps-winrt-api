---
-api-id: E:Microsoft.Windows.PushNotifications.PushNotificationReceivedEventArgs.Canceled
-api-type: winrt event
---

# Microsoft.Windows.PushNotifications.PushNotificationReceivedEventArgs.Canceled

<!--
public event Windows.ApplicationModel.Background.BackgroundTaskCanceledEventHandler Canceled;
-->


## -description

Raised when the system is going to cancel the background task launched to handle the [PushReceived](/windows/windows-app-sdk/api/winrt/microsoft.windows.pushnotifications.pushnotificationmanager.pushreceived) event. 

## -remarks

For information on the reasons that the system may cancel a background task, see [BackgroundTaskCancellationReason](/uwp/api/windows.applicationmodel.background.backgroundtaskcancellationreason).

## -see-also

## -examples


