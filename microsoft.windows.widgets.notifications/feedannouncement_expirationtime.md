...
The **ExpirationTime** and <xref:Microsoft.Windows.Widgets.Notifications.FeedAnnouncement.Duration> are related in such a way that, at the time an announcement secures a spot for display, the current time plus the duration must not exceed the expiration time.
