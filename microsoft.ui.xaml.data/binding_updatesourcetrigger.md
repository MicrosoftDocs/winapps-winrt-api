...
The default behavior for most dependency properties is `PropertyChanged`. However, the default behavior for the [TextBox.Text](../microsoft.ui.xaml.controls/textbox_text.md) property is `LostFocus`. For a [Binding](binding.md) to `TextBox.Text`, you can change the `UpdateSourceTrigger` like this:

```xaml
<TextBox Text="{x:Bind MyProperty, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}" />
```

However, it won’t have any effect on a `TextBox` in the control template of another control, such as [NumberBox](../microsoft.ui.xaml.controls/numberbox.md). For example, this `UpdateSourceTrigger` setting has no effect.
